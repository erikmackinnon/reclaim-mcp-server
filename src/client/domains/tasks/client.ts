...
async function fetchCurrentUser(): Promise<CurrentUserResponse | undefined> {
  ...
  if (!cachedCurrentUserPromise) {
    cachedCurrentUserPromise = (async () => {
      const data = await reclaimHttpClient.get<CurrentUserResponse>(
        "/users/current",
...
export async function listTasks(): Promise<Task[]> {
  ...
  const data = await reclaimHttpClient.get<Task[]>("/tasks", { context });
...
export async function getTask(taskId: number): Promise<Task> {
  ...
  return reclaimHttpClient.get<Task>(`/tasks/${taskId}`, { context });
}
...
export async function createTask(
  ...
  let apiPayload: Partial<TaskInputData>;
  ...
  try {
    ...
    apiPayload = { ...normalizedInput }; // Clone to avoid modifying input object
  ...
  return reclaimHttpClient.post<Task>("/tasks", apiPayload, { context });
}
...
export async function createTaskAtTime(
  ...
  try {
    ...
    startTimeIso = parseDeadline(startTime, {
      timeZone: resolvedTimeZone,
    });
    apiPayload = { ...normalizedInput };
  ...
  return reclaimHttpClient.post<any>("/tasks/at-time", apiPayload, {
    context,
    query: { startTime: startTimeIso },
  });
}
...
export async function updateTask(
  ...
  try {
    ...
    apiPayload = normalizeTaskPatch(taskData, defaults);
  ...
  debugLog(`${context} payload`, apiPayload);
  return reclaimHttpClient.patch<Task>(`/tasks/${taskId}`, apiPayload, {
    context,
  });
}
...
export async function markTaskComplete(taskId: number): Promise<any> {
  ...
  const data = await reclaimHttpClient.post<any>(
    `/planner/done/task/${taskId}`,
    undefined,
    { context },
  );
...
export async function logWorkForTask(
  ...
  let params: { minutes: number; end?: string };
...
export async function prioritizeTask(taskId: number): Promise<any> {
  ...
  const data = await reclaimHttpClient.post<any>(
    `/planner/prioritize/task/${taskId}`,
    undefined,
    { context },
  );
  return data ?? { success: true };
